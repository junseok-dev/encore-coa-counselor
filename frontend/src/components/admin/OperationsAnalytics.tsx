...
export default function OperationsAnalytics({ data, loading, onRefresh }: OperationsAnalyticsProps) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-2xl bg-[linear-gradient(120deg,#172554,#0e7490)] px-6 py-5 text-white shadow-lg sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div><p className="text-sm font-semibold text-cyan-100">운영 데이터 분석</p><h1 className="mt-1 text-2xl font-black">방문·채팅 이용 패턴</h1><p className="mt-1 text-xs text-blue-100">월별 추이와 최근 {data?.hourly_days ?? 90}일 시간대 패턴을 분리해 봅니다.</p></div>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <PeakCard label="방문자가 가장 많았던 달" value={monthTitle(highlights?.busiest_visitor_month?.label)} count={highlights?.busiest_visitor_month?.count} unit="명" icon={CalendarDays} color="text-violet-600" />
        <PeakCard label="채팅이 가장 많았던 달" value={monthTitle(highlights?.busiest_chat_month?.label)} count={highlights?.busiest_chat_month?.count} unit="건" icon={MessageCircle} color="text-cyan-600" />
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-bold text-slate-950">월별 방문자·채팅 수</h2><p className="mt-1 text-xs text-slate-500">최근 {data?.period_months ?? 12}개월 기준입니다.</p></div><div className="flex gap-4 text-xs font-bold text-slate-500"><span className="flex items-center gap-1.5"><span className="h-2.5 w-4 rounded bg-violet-600" />방문자</span><span className="flex items-center gap-1.5"><span className="h-2.5 w-4 rounded bg-cyan-600" />채팅</span></div></div>
        <div className="mt-4">{data ? <MonthlyChart data={data.monthly} /> : <p className="py-20 text-center text-sm text-slate-400">{loading ? '분석 중입니다.' : '분석 데이터가 없습니다.'}</p>}</div>
      </section>

      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-bold text-slate-950">시간대별 방문·채팅 패턴</h2><p className="mt-1 text-xs text-slate-500">최근 {data?.hourly_days ?? 90}일 누적 기준이며, 00시부터 23시까지 표시합니다.</p></div><div className="flex gap-4 text-xs font-bold text-slate-500"><span className="flex items-center gap-1.5"><span className="h-2.5 w-4 rounded bg-violet-500" />방문자</span><span className="flex items-center gap-1.5"><span className="h-2.5 w-4 rounded bg-cyan-600" />채팅</span></div></div>
        <div className="mt-4">{data ? <HourlyChart data={data.hourly} /> : <p className="py-20 text-center text-sm text-slate-400">시간대 데이터를 불러오는 중입니다.</p>}</div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{(data?.handoff_categories ?? []).map((item) => <div key={item.key}><div className="mb-1.5 flex justify-between text-sm"><span className="font-medium text-slate-600">{item.label}</span><span className="font-black text-slate-950">{item.count}</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-violet-500" style={{ width: `${item.count ? Math.max(6, (item.count / handoffMax) * 100) : 0}%` }} /></div></div>)}</div>
      </section>
    </div>
  );
}
