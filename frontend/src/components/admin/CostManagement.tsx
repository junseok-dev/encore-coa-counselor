...
export default function CostManagement() {
  ...
  const load = async (month = billingMonth, account = accountId) => {
    setLoading(true);
    try {
      const result = await adminApi.getCostManagement(month, account);
      setData(result);
  ...
  useEffect(() => { void load(); }, [billingMonth, accountId]);
  ...
  const importFile = async () => {
    ...
    setImporting(true);
    try {
      const result = await adminApi.importBillingCosts(file, billingMonth, importAccountId, importAccountName);
      setMessage(`${result.imported_rows.toLocaleString()}개 일별 비용 항목을 반영했습니다.`);
      await load(billingMonth, 'all');
      setAccountId('all');
    } catch {
      setMessage('파일을 반영하지 못했습니다. 열 이름과 계정 정보를 확인하세요.');
    } finally {
      setImporting(false);
    }
  };
  ...
  const downloadTemplate = async () => {
    const blob = await adminApi.downloadCostTemplate();
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement('a');
    anchor.href = url;
    anchor.download = 'cost_import_template.xlsx';
    anchor.click();
  ...
  return (
    <div className="space-y-6">
      <div className="overflow-hidden rounded-2xl bg-[linear-gradient(120deg,#0f172a,#1d4ed8)] px-6 py-5 text-white shadow-lg">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-center gap-4"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10 ring-1 ring-white/15"><WalletCards className="h-6 w-6 text-blue-200" /></span><div><p className="text-sm font-semibold text-blue-100">실제 원화 기준</p><h1 className="mt-1 text-2xl font-black">비용 관리</h1><p className="mt-1 text-xs text-blue-100">n·Xavis 청구 자료를 계정·서비스·일자별로 확인합니다.</p></div></div><div className="flex flex-wrap gap-2"><label className="rounded-xl bg-white/10 px-3 py-2 text-xs font-bold ring-1 ring-white/20">월 <input type="month" value={billingMonth} onChange={(event) => { setBillingMonth(event.target.value); setAccountId('all'); }} className="ml-2 bg-transparent outline-none" /></label><select value={accountId} onChange={(event) => setAccountId(event.target.value)} className="rounded-xl bg-white px-3 py-2 text-xs font-bold text-slate-700"><option value="all">전체 계정</option>{data?.accounts.map((account) => <option key={account.account_id} value={account.account_id}>{account.account_name} · {account.account_id}</option>)}</select><button onClick={() => void load()} className="rounded-xl bg-white/10 p-2.5 ring-1 ring-white/20"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /></button></div></div>
      </div>

      {message && <div className="rounded-xl bg-blue-50 px-4 py-3 text-sm font-semibold text-blue-700">{message}</div>}

      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-bold text-slate-500">전체 계정 실제 청구액</p><p className="mt-2 text-3xl font-black text-slate-950">{krw(data?.actual_invoice_krw)}</p><div className="mt-4 flex gap-2"><input value={invoiceAmount} onChange={(event) => setInvoiceAmount(event.target.value.replace(/[^0-9]/g, ''))} placeholder="실제 청구액 입력" className="min-w-0 flex-1 rounded-lg border border-slate-200 px-3 py-2 text-sm" /><button onClick={() => void saveInvoice()} disabled={!invoiceAmount} className="rounded-lg bg-slate-950 px-3 py-2 text-xs font-bold text-white disabled:opacity-40">저장</button></div></div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-bold text-slate-500">선택 범위 서비스 사용 합계</p><p className="mt-2 text-3xl font-black text-blue-700">{krw(data?.usage_total_krw)}</p><p className="mt-3 text-xs text-slate-400">{accountId === 'all' ? '전체 계정' : '선택 계정'} · 일별 서비스 합산</p></div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-bold text-slate-500">청구액과 사용 합계 차이</p><p className={`mt-2 text-3xl font-black ${(data?.difference_krw ?? 0) === 0 ? 'text-emerald-600' : 'text-amber-600'}`}>{krw(data?.difference_krw)}</p><p className="mt-3 text-xs text-slate-400">세금·할인·조정액 확인용이며 전체 계정 선택 시 계산합니다.</p></div>
      </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><FileSpreadsheet className="h-5 w-5 text-emerald-600" /><div><h2 className="font-bold text-slate-950">n·Xavis 원화 자료 반영</h2><p className="mt-1 text-xs text-slate-500">표준 행 형식 또는 서비스×일자 행렬 형식의 XLSX·CSV를 지원합니다.</p></div></div><div className="mt-5 grid gap-3 lg:grid-cols-[1fr_1fr_1.4fr_auto_auto] lg:items-end"><label className="text-xs font-bold text-slate-600">계정 ID<input value={importAccountId} onChange={(event) => setImportAccountId(event.target.value)} placeholder="2491..." className="mt-2 block w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm" /></label><label className="text-xs font-bold text-slate-600">계정명<input value={importAccountName} onChange={(event) => setImportAccountName(event.target.value)} placeholder="엔코아 동작캠퍼스" className="mt-2 block w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm" /></label><label className="text-xs font-bold text-slate-600">비용 파일<input type="file" accept=".xlsx,.csv" onChange={(event) => setFile(event.target.files?.[0] ?? null)} className="mt-2 block w-full text-sm" /></label><button onClick={() => void downloadTemplate()} className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-bold text-slate-600"><Download className="h-4 w-4" />양식</button><button onClick={() => void importFile()} disabled={!file || importing} className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-bold text-white disabled:opacity-40"><Upload className="h-4 w-4" />{importing ? '반영 중' : '업로드'}</button></div></section>
    </div>
  );
}
