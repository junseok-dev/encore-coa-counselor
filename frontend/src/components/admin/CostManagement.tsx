...
export default function CostManagement() {
  ...
  return (
    <div className="space-y-6">
      ...
      {costTab === 'openai' && <section className="flex flex-col gap-4 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-start gap-3"><span className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-emerald-100 text-emerald-700"><Info className="h-5 w-5" /></span><div><h2 className="text-sm font-black text-emerald-950">OpenAI 비용 확인 경로</h2><p className="mt-1 text-xs leading-5 text-emerald-800"><strong>OpenAI API Platform → Usage</strong>에서 <strong>AIcampus_Chatbot</strong> API 항목을 선택한 뒤, 조회할 월의 실제 사용 금액(USD)을 아래에 입력합니다. 자동 연동되지 않으므로 월별로 직접 확인해 저장해야 합니다.</p></div></div>
        <a href="https://platform.openai.com/usage" target="_blank" rel="noreferrer" className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-emerald-700 px-4 py-2.5 text-xs font-black text-white hover:bg-emerald-800">OpenAI Usage 열기<ExternalLink className="h-4 w-4" /></a>
      </section>}
      ...
      {costTab === 'openai' && <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-slate-200 bg-slate-50 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div><h2 className="font-black text-slate-950">OpenAI API 실제 비용</h2><p className="mt-0.5 text-xs text-slate-500">Usage의 AIcampus_Chatbot API에서 확인한 월 금액을 직접 기록합니다.</p></div>
          </div>
