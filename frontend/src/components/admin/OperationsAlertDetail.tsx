...
export default function OperationsAlertDetailPanel({
  ...
  const [copiedDeveloperPrompt, setCopiedDeveloperPrompt] = useState<string | null>(null);
  ...
  const saveWorkflow = async (nextStatus: OperationsAlertStatus): Promise<boolean> => {
    setSaving(true);
    setError('');
    try {
  ...
  const keepCurrentAnswer = async () => {
    ...
    setError('');
    try {
      const result = await adminApi.keepOperationsAlertAnswer(item.alert_id);
  ...
  const copyDeveloperPrompt = async (prompt: string, key: string) => {
    try {
      await navigator.clipboard.writeText(prompt);
      setCopiedDeveloperPrompt(key);
      window.setTimeout(() => setCopiedDeveloperPrompt((current) => current === key ? null : current), 2000);
    } catch {
      setError('개발자 전달 프롬프트를 복사하지 못했습니다. 브라우저의 클립보드 권한을 확인해 주세요.');
    }
  };
        ...
        <header className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-slate-200 bg-white px-5 py-4 sm:px-7">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className={`rounded-full px-2 py-1 text-[10px] font-bold ${status === 'resolved' ? 'bg-emerald-100 text-emerald-700' : status === 'developer_required' ? 'bg-amber-100 text-amber-900' : status === 'checking' ? 'bg-blue-100 text-blue-700' : 'bg-rose-100 text-rose-700'}`}>
                {status === 'resolved' ? '처리 완료' : status === 'developer_required' ? '개발자 조치 필요' : status === 'checking' ? '확인 중' : '미확인'}
              </span>
            </div>
              ...
              <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                {status === 'developer_required' ? (
                  <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-4">
                    <p className="flex items-center gap-2 text-sm font-black text-amber-950"><Wrench className="h-4 w-4" />개발자 조치가 필요한 문제입니다.</p>
                    <p className="mt-2 text-xs leading-5 text-amber-900">아래 관리자 AI 대화에서 개발 도구 AI 전달용 프롬프트를 확인·복사하세요. 코드 수정과 배포가 끝나면 이 화면에서 같은 질문으로 검증합니다.</p>
                  </div>
                ) : (
                  <>
                    <p className="mt-2 text-xs leading-5 text-slate-500">원인에 따라 답변 프롬프트를 수정하고 변경 전후를 검증한 뒤 운영에 반영하세요.</p>
                    ...
                    <div className="mt-4 rounded-xl bg-amber-50 p-3 text-xs leading-5 text-amber-800">수정·운영 반영 후 같은 질문으로 결과를 확인하세요.</div>
                  </>
                )}
              </section>
            </div>
                  ...
                  <div className="min-w-0 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                      ...
                      {detail.ai_messages.map((message) => (
                        <div key={message.id} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                          <div className={`max-w-[90%] rounded-2xl px-3.5 py-2.5 text-xs leading-5 ${message.role === 'user' ? 'bg-slate-900 text-white' : 'border border-violet-100 bg-violet-50 text-slate-700'}`}>
                            ...
                            {message.structured?.root_cause === 'code' && message.structured.developer_handoff_prompt && (
                              <details className="mt-2 rounded-lg border border-amber-200 bg-amber-50 p-2 text-amber-950">
                                <summary className="cursor-pointer font-black">개발 도구 AI 전달 프롬프트</summary>
                                <pre className="mt-2 max-h-64 overflow-auto whitespace-pre-wrap break-words font-sans text-[11px] leading-5 text-slate-700">{message.structured.developer_handoff_prompt}</pre>
                                <button type="button" onClick={() => void copyDeveloperPrompt(message.structured!.developer_handoff_prompt, `message-${message.id}`)} className="mt-2 inline-flex items-center gap-1.5 rounded-lg bg-amber-900 px-2.5 py-1.5 text-[11px] font-black text-white">
                                  <ClipboardCopy className="h-3.5 w-3.5" />{copiedDeveloperPrompt === `message-${message.id}` ? '복사됨' : '프롬프트 복사'}
                                </button>
                              </details>
                            )}
                    ...
                    {latestAiAnalysis && (
                      <div className="mt-3 rounded-xl border border-violet-200 bg-white p-3">
                        ...
                        {latestAiAnalysis.root_cause === 'code' && latestAiAnalysis.developer_handoff_prompt && (
                          <div className="mt-3 rounded-xl border border-amber-300 bg-amber-50 p-3">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              <p className="flex items-center gap-1.5 text-xs font-black text-amber-950"><Wrench className="h-4 w-4" />개발 도구 AI 전달 프롬프트</p>
                              <button type="button" onClick={() => void copyDeveloperPrompt(latestAiAnalysis.developer_handoff_prompt, 'latest')} className="inline-flex items-center gap-1.5 rounded-lg bg-amber-900 px-2.5 py-1.5 text-[11px] font-black text-white">
                                <ClipboardCopy className="h-3.5 w-3.5" />{copiedDeveloperPrompt === 'latest' ? '복사됨' : '복사'}
                              </button>
                            </div>
                            <pre className="mt-2 max-h-72 overflow-auto whitespace-pre-wrap break-words rounded-lg bg-white p-3 font-sans text-[11px] leading-5 text-slate-700">{latestAiAnalysis.developer_handoff_prompt}</pre>
                          </div>
                        )}
