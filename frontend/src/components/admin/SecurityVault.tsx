...
import { Copy, ExternalLink, Eye, EyeOff, KeyRound, Lock, Save, ShieldCheck } from 'lucide-react';
...
export default function SecurityVault() {
  ...
  if (!vaultToken || !data) {
    ...
    return (
      <div className="mx-auto max-w-xl rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        {needsSetup && !status.can_setup ? <div className="mt-6 rounded-xl bg-rose-50 px-4 py-3 text-sm text-rose-700">최상위 관리자만 최초 보관 비밀번호를 설정할 수 있습니다.</div> : <div className="mt-6 space-y-4"><label className="block text-sm font-bold text-slate-700">보관 비밀번호<input autoComplete="new-password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !needsSetup) void unlock(); }} placeholder="12자 이상" className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-blue-500" /></label>{needsSetup && <label className="block text-sm font-bold text-slate-700">비밀번호 확인<input autoComplete="new-password" type="password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-blue-500" /></label>}<button onClick={() => void (needsSetup ? setup() : unlock())} disabled={loading || password.length < (needsSetup ? 12 : 1) || (needsSetup && !confirmPassword)} className="w-full rounded-xl bg-slate-950 px-4 py-3 text-sm font-black text-white disabled:opacity-40">{loading ? '확인 중...' : needsSetup ? '보관 비밀번호 설정' : '잠금 해제'}</button>{needsSetup && <p className="text-xs leading-5 text-slate-400">비밀번호는 12자 이상이며 문자·숫자·특수문자 중 2종 이상을 포함해야 합니다. 분실 시 화면에서 복구할 수 없습니다.</p>}</div>}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <section className="flex flex-col gap-4 rounded-3xl bg-[linear-gradient(120deg,#0f172a,#1e3a8a)] p-6 text-white shadow-lg sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-4"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10"><ShieldCheck className="h-6 w-6 text-blue-200" /></span><div><h2 className="text-xl font-black">운영 보안 정보</h2><p className="mt-1 text-xs text-blue-100">잠금 해제된 동안 운영 접속정보와 허용된 환경설정을 확인할 수 있습니다.</p></div></div><div className="flex items-center gap-3"><span className="rounded-xl bg-white/10 px-3 py-2 font-mono text-xs ring-1 ring-white/20">자동 잠금 {expiryLabel}</span><button onClick={() => lockVault()} className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-xs font-black text-slate-900"><Lock className="h-4 w-4" />잠그기</button></div></section>
