...
- **어떻게**:
  ...
  - 라이브 점검 반영: 혼란 표현("뭔소리야·엥")은 상담사 연결 대신 **직전 대화 맥락을 보고 더 쉽게 재설명**(라우터가 rag로) — 단 비유·예시는 1회만 하고 그 뒤엔 실제 상담으로 진전; 답할 수 없는 통계(평균 나이·입사자 수)는 "왜 못 하는지(자료 없음 등)" 메타 없이 "안내 어렵다 + 필요 시 매니저 연결"; 나이 제한 여부는 "제한 없음" 단정; 민감 특성(나이 숫자·실버타운 등)은 되받아 강조 금지; 오타·축약은 맥락으로 의도 추론; FAQ 선택지의 빈 괄호 placeholder 제거.
