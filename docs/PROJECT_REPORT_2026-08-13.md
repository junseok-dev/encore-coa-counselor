# 엔코아 AI 캠퍼스 상담 챗봇 — 기술 보고서

> 교육 과정 안내 및 상담을 위한 문서 기반 RAG 챗봇 시스템

> 기준일: 2026-08-13  
> 이 문서는 기존 상세 README를 보존한 기술 보고서입니다. 빠른 실행과 핵심 결과는 [루트 README](../README.md)를 참고하세요.

## 2026-08 운영 현황 요약

- 운영 대시보드에서 상담 연결·취소·환불·홈페이지 유입·안전·오류 신호를 기간별로 집계합니다.
- 긴급 확인 큐에서 원문 대화, 관련 알림, 담당자, 처리 상태와 답변 재시험 이력을 관리합니다.
...
- **상담 모드 자동 분리** — 사실 질문은 짧고 직접적으로, 상황 판단은 판단·근거·현실적 조건까지, 과정 추천은 잠정 추천 후 핵심 확인질문 1개와 최종 추천으로 이어짐
- **결론 먼저·필요한 만큼** — 단순 정보는 짧게 답하되 추천·상황 상담은 결정에 필요한 차이와 준비 방향까지 안내. 반복되거나 가치가 낮은 되묻기는 차단
...
- **민감한 취업 상담 분리** — 특정 기업 합격이나 취업 결과를 보장하지 않으며, 취업 책임 안내 뒤 책임 공방이 이어지면 추가 생성 답변 없이 상담 매니저에게 연결
...
[2] 하이브리드 라우터 (router_service.route)
    ...
    ├─ 과정 추천: 세 과정 적합성 비교가 가능한 RAG로 결정적 분기
...
[3] handler 분기
    ...
    guide        → source="faq"      (카테고리 메뉴) — 단, 대화 맥락이 있으면 rag로 강등(후속답변 보호)
        │
        ▼
[4] 상담 모드 적용 (consultation_service.py)
    fact          → 질문의 사실을 짧고 직접적으로 답변
    situation     → 판단 + 근거 + 현실적인 조건을 함께 안내
    recommendation→ 잠정 추천 + 이유/차이 + 필요한 경우 핵심 질문 1개
                    다음 턴에는 최종 추천 + 대안 + 준비 방향으로 종료
        │
        ▼
[5] 후처리
    ├─ 본문 URL/채널톡 마크다운 링크 sanitize, "채널톡" 언급 시 source="handoff" 승격
    ├─ 굵은 제목만 말풍선 분리 대상으로 처리해 일반 목록은 한 말풍선에 유지
...
│   │   │   ├── consultation_service.py ← 사실·상황·추천 상담 모드 분류와 응답 지침
│   │   │   ├── employment_service.py ← 취업 책임 안내와 후속 불만 상담 이관 판정
...
│   ├── tests/
│   │   ├── test_consultation_service.py ← 상담 모드·추천 후속 흐름 테스트
│   │   ├── test_employment_service.py ← 취업 책임 후속 이관 테스트
│   │   ├── test_response_formatter.py ← 목록 말풍선 분리 회귀 테스트
...
각각 백엔드·프론트 터미널 두 개가 자동으로 열립니다. 브라우저로 <http://localhost:5174> 접속.
...
| 답변 스타일   | 캐묻기형 또는 안전 규칙이 누적된 방어적 답변      | 사실·상황·추천 모드 분리 + 잠정 판단 후 핵심 질문 1회           | 짧은 답변과 깊이 있는 상담의 균형             |
...
- **어떻게**: 단순 사실 질문에는 추가 질문을 붙이지 않고, 과정 추천에는 잠정 추천을 먼저 제시한 뒤 결정에 실제로 필요한 핵심 질문을 최대 1개만 허용한다. 사용자가 답한 다음 턴에는 추가 확인질문을 막고 최종 추천·대안·준비 행동으로 닫는다(`NO_REASK_DIRECTIVE`).
...
### 9) 상담 깊이 복원 — 전역 안전형 답변 → 모드별 능동 상담 (2026-07)

- **문제**: 짧은 답변, 질문 금지, 확인되지 않은 정보 제한 같은 규칙이 모든 질문에 동일하게 적용되면서 단순 정보는 안전해졌지만 과정 추천과 상황 판단까지 방어적으로 바뀌었다. 사용자의 조건을 해석하지 않고 과정 목록만 보여주거나, 정확한 수치가 없다는 이유만으로 상담 연결을 제안하는 문제가 있었다.
- **상담 모드**: `consultation_service.py`가 질문을 `fact`, `situation`, `recommendation`으로 분류한다. 사실 질문은 직접 답하고, 상황 질문은 현재 조건에 대한 판단과 현실적인 변수를 설명한다. 추천 질문은 세 과정 문서를 함께 검색해 가장 적합한 과정을 먼저 제시한다.
- **추천 흐름**: 첫 답변에서 잠정 추천·이유·가장 가까운 대안의 차이를 안내한 뒤, 결과를 실제로 바꿀 수 있는 구체적인 질문만 최대 1개 묻는다. 다음 턴에는 질문을 반복하지 않고 최종 추천, 대안, 준비 방향과 다음 행동까지 제시한다.
- **안전 경계**: 특정 기업 취업 여부나 합격 가능성은 근거 없이 긍정하지 않는다. 취업 책임 질문에는 교육기관이 제공해야 할 범위와 채용 결과의 관계를 한 번 안내하고, 사용자가 책임 공방을 이어가면 표현을 바꿔 반복 답변하지 않고 상담 매니저에게 연결한다.
- **불필요한 이관 축소**: 취업률처럼 확정 수치를 안내할 수 없는 질문도 곧바로 상담 매니저에게 넘기지 않는다. 수치를 임의로 만들지 않으면서 확인된 채용 연계·이력서/포트폴리오 피드백·기술면접 컨설팅·우수 수료생 추천 제도를 먼저 안내한다.
- **말풍선 정리**: 취업지원 프로그램 같은 일반 목록은 하나의 말풍선에서 읽히도록 유지하고, 굵은 글씨가 포함됐다는 이유만으로 항목마다 분리하지 않는다. 독립된 굵은 제목 줄만 새 말풍선의 경계로 인식한다.
- **검증**: 상담 모드 분류·추천 후속 종료·책임 공방 이관·목록 말풍선 처리를 포함한 백엔드 단위 테스트 **22개**를 통과했다.
